...
class ScreenState(object):
    ...
    def switchTo(self,clearEvents=True,msg=None):
        ...
        if deviceEventTriggers and len(deviceEventTriggers)>0:
            ...
            ER.hub.delay(self.timeout-0.002), None
            localClearEvents('all')
...
class ImageScreen(ScreenState):
    #TODO: test
    def __init__(self, experimentRuntime, imageName, imagePos=(0,0), imageSize=None, deviceEventTriggers=None, timeout=None):
        ScreenState.__init__(self, experimentRuntime, deviceEventTriggers, timeout)
        w, h = self.experimentRuntime().devices.display.getStimulusScreenResolution()
        ...
        self.stim['IMAGE'] = psychopyVisual.ImageStim(self.window(), image=imageName, pos=imagePos, size=imageSize,name=imageName)
