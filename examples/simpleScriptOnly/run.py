...
def runExample(experimentScriptDir,configurationFile=None):
    ...
    print "You played around with the mouse cursor for {0} seconds.".format(k['hub_time']-first_flip_time)
    print ''
...
if __name__ == "__main__":
    import sys
    experimentScriptDir=ioHub.module_directory(runExample)
    if len(sys.argv)>1:
        configFileName=unicode(sys.argv[1])
        runExample(experimentScriptDir,configFileName)
    else:
        runExample(experimentScriptDir)
