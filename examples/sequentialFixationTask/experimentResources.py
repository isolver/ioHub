...
class TargetScreen(ScreenState):
    ...
    def __init__(self,experimentRuntime, deviceEventTriggers=None, timeout=None):
        ...
        self.stim['OUTER_POINT']=visual.Circle(self.window(),radius=(self.TARGET_OUTER_RADIUS,self.TARGET_OUTER_RADIUS),lineWidth=0, lineColor=None, lineColorSpace='rgb255', name='FP_OUTER', opacity=1.0, interpolate=False, units='pix',pos=(0,0))
        self.stimNames.append('OUTER_POINT')
        self.stim['OUTER_POINT'].setFillColor(self.TARGET_OUTER_COLOR,'rgb255')

        self.stim['INNER_POINT']=visual.Circle(self.window(),radius=(self.TARGET_INNER_RADIUS,self.TARGET_INNER_RADIUS),lineWidth=0, lineColor=None, lineColorSpace='rgb255', name='FP_INNER', opacity=1.0, interpolate=False, units='pix', pos=(0,0))
        ...
        ppd=experimentRuntime.devices.display.getPPD()
        print 'PPD reply: ', ppd
        self.stim['DYNAMIC_STIM']=visual.GratingStim(self.window(),tex=None, mask="gauss", pos=[0,0],size=experimentRuntime.devices.display.getPPD(),color='purple',opacity=0.0)
