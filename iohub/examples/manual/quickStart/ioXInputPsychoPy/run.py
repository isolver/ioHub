...
class ExperimentRuntime(ioHubExperimentRuntime):
    ...
    def run(self,*args,**kwargs):
        ...
        while not END_DEMO:
            ...
            x,y,mag=gamepad.getThumbSticks()['right_stick'] # sticks are 3 item lists (x,y,magnitude)
            ...
            for event in keyboard.getEvents(EventConstants.KEYBOARD_PRESS):
                if event.key in ['q',]:                
                    END_DEMO=True
                elif event.key in ['r',]:
                    print 'RUMBLE CALLED'
            ...
            self.hub.clearEvents('all')#do this each frame to avoid keyboard event buffer filling with non press event types

    def normalizedValue2Coord(self,normalized_position,normalized_magnitude,display_coord_dim_size):
