...
class Display(Device):
    _coord_type_mappings=dict( org='org', pix='pix', pixel='pix', pixels='pix',
    ...
    _pixel_coord_type_alias=['pix','pixel','pixels']    
    EVENT_CLASS_NAMES=[]
    ...
    def _addRuntimeInfoToDisplayConfig(self):
        ...
        if runtime_info is None:        
            ...
            self. _calculateCoordMappingFunctions(pixel_width,pixel_height,phys_unit_type, phys_width,phys_height, degree_width, degree_height)
    ...
    def _calculateCoordMappingFunctions(self,pixel_width,pixel_height,phys_unit_type, phys_width,phys_height, degree_width, degree_height):
        ...
        if coord_type in Display._coord_type_mappings:
            ...
            ioHub.print2err(" *** Display device error: Unknown coordinate type: {0}".format(coord_type))
            return
        
        origin=self.getOrigin()
        if origin not in Display._supported_origin_types:   
            ioHub.print2err(" *** Display device error: Unknown origin type: {0}".format(origin))
            return
    ...
    def _createPsychopyCalibrationFile(self):
        ...
        #TODO: BETTER INTEGRATE Display classs with PsychoPy Monitor file
