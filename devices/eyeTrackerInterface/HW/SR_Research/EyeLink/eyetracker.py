...
from .... import RTN_CODES, DATA_STREAMS, EyeTrackerInterface
...
class EyeTracker(EyeTrackerInterface):
    ...
    _eyeTrackerConfig=None # holds the configuration / settings dict for the device
    ...
    def __init__(self,*args,**kwargs):
        ...
        **If an instance of EyeTracker has already been created, trying to create a second will raise an exception.
        ...
        deviceSettings={
            ...
            'name':EyeTracker._eyeTrackerConfig['name'],
            'os_device_code':'OS_DEV_CODE_NOT_SET',
            'max_event_buffer_length':EyeTracker._eyeTrackerConfig['event_buffer_length']
            }
        ...
        dummyModeEnabled=EyeTracker._eyeTrackerConfig['runtime_settings'].get('enable_api_without_connection',False)
        if dummyModeEnabled:
            self._eyelink.dummy_open()

        EyeTracker._eyelink=pylink.EyeLink(EyeTracker._eyeTrackerConfig['runtime_settings'].get('network_settings','100.1.1.1'))
        ...
        try:
            r=self._eyelinkSetScreenPhysicalData()
        except:
    ...
    def _fileTransferProgressUpdate(self,size,received):
        from experiment import ProgressBarDialog
    ...
    def sendCommand(self, *args, **kwargs):
        ...
        if command in EyeTracker._COMMAND_TO_FUNCTION:
            r=EyeTracker._COMMAND_TO_FUNCTION[command](*args)
        else:
            cmdstr=''
            if len(args)==0:
    ...
    def getEyesToTrack(self,*args,**kwargs):
        ...
        if eye_used == RIGHT_EYE:
            return EventConstants.RIGHT
        elif eye_used == LEFT_EYE:
            return EventConstants.LEFT
        elif eye_used == BINOCULAR:
            return EventConstants.BINOCULAR
        else:
        ...
        if r == "BOTH":
            if self._eyelink.getTrackerVersion() == 3:
                if srate>=1000:
                    ...
                    trackerVersion =self.getTrackerVersionString().strip()
    ...
    def getDataFilterLevel(self,*args,**kwargs):
        ...
        if 'data_stream' in kwargs:
            ...
            data_stream=args[0]
        ioHub.print2err('WARNING: getDataFilterLevel is not implemented yet.')
        return False
    ...
    def _poll(self):
        ...
        try:
            ...
            confidenceInterval=poll_time-EyeTracker._lastPollTime
            EyeTracker._lastPollTime=poll_time
            ...
            while 1:
                ...
                hub_timestamp=cltime-event_delay

                ne.logged_time=cltime
                ne.event_timestamp=event_timestamp
                ne.hub_timestamp=hub_timestamp
            ...
            for ne in nEvents:
                if isinstance(ne,pylink.Sample):
                    ...
                    if ne.isBinocular():
                        # binocular sample
                        event_type=EventConstants.EVENT_TYPES['BINOC_EYE_SAMPLE']
                    ...
                    etype=EventConstants.EVENT_TYPES['FIXATION_END']
                    ...
                    etype=EventConstants.EVENT_TYPES['SACCADE_END']
                    ...
                    etype=EventConstants.EVENT_TYPES['BLINK_END']
                    ...
                    etype=EventConstants.EVENT_TYPES['FIXATION_START']

                    if isinstance(ne,pylink.StartSaccadeEvent):
                        etype=EventConstants.EVENT_TYPES['SACCADE_START']
                    ...
                    pupil_size=-1
                    if etype == EventConstants.EVENT_TYPES['FIXATION_START']:
                    ...
                    se=[
                        ...
                        gaze[0],                                # gaze x
                        gaze[1],                                # gaze y
                        EventConstants.NOT_SUPPORTED_FIELD,                                     # gaze z
                        href[0],                                # angle x
                        href[1],                                # angle y
                        EventConstants.NOT_SUPPORTED_FIELD,                                   # raw x
                        EventConstants.NOT_SUPPORTED_FIELD,                                   # raw y
                        pupil_size,                             # pupil area
                        EventConstants.AREA,                    # pupil measure type 1
                        EventConstants.NOT_SUPPORTED_FIELD,                                   # pupil measure 2
                        EventConstants.NOT_SUPPORTED_FIELD,     # pupil measure 2 type
                        ppd[0],                                 # ppd x
                        ppd[1],                                 # ppd y
                        EventConstants.NOT_SUPPORTED_FIELD,                                    # velocity x
                        EventConstants.NOT_SUPPORTED_FIELD,                                    # velocity y
            ...
            EyeTracker._lastPollTime=pollEndLocalTime
        except Exception, e:
            ioHub.printExceptionDetailsToStdErr()
