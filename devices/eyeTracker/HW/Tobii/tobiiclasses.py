# -*- coding: utf-8 -*-
"""
Created on Fri Jan 25 07:50:36 2013
...
class TobiiTrackerBrowser(object):
    _mainloop = None
    _detectedTrackers = ioHub.OrderedDict()
    _browser=None
    _event_queue=None
    _active=False
    ...
    def stop(cls,empty_queue=True):
        if cls._active:        
            ...
            TobiiTrackerBrowser._detectedTrackers=ioHub.OrderedDict()
    ...
    def on_eyetracker_browser_event(cls, event_type, event_name, eyetracker_info):
        ...
        raise ioHub.devices.ioDeviceError("TobiiTrackerBrowser.on_eyetracker_browser_event received unhandled event type",event_type,eyetracker_info)
...
class TobiiTracker(object):
    LEFT=0
    RIGHT=1
    eye_data=ioHub.OrderedDict(tracker_time_usec=np.NaN,
    ...
    def on_eyetracker_error(self, *args, **kwargs):
        ioHub.print2err("TobiiTracker.on_eyetracker_error: ",args, kwargs)
        return False

    def on_sync_error(self, *args, **kwargs):
        ioHub.print2err("TobiiTracker.on_sync_error: ",args, kwargs)
        return False
    ...
    def on_external_framerate_change(self, *args,**kwargs):
        ioHub.print2err("NOTE: Tobii System Sampling Rate Changed.")        
        return False   

    def on_head_box_change(self, *args,**kwargs):
        ioHub.print2err("NOTE: Tobii Head Movement Box Changed.")        
        return False
    
    def on_x_series_physical_config_change(self, *args, **kwargs):
        ioHub.print2err("NOTE: Tobii X Series Physical Settings Changed.")        
        return False
    ...
    def getAvailableExtensions(self):
        return [ioHub.OrderedDict(name=e.Name,extensionId=e.ExtensionId,protocolVersion=e.ProtocolVersion,realm=e.Realm) for e in self._eyetracker.GetAvailableExtensions()]

    def getEnabledExtensions(self):
        return [ioHub.OrderedDict(name=e.Name,extensionId=e.ExtensionId,protocolVersion=e.ProtocolVersion,realm=e.Realm) for e in self._eyetracker.GetEnabledExtensions()]
    
    def enableExtension(self,extension):
        extension_id=extension
        if isinstance(extension, ioHub.OrderedDict):
