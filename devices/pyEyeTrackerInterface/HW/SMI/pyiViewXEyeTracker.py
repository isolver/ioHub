"""
pyiViewXEyeTracker: SMI iViewX interface implemtation of pyEyeTracker

Part of the pyEyeTracker library 
Distributed under the terms of the GNU General Public License (GPL version 3 or any later version). 

pyiViewXEyeTracker uses the UDF ASCII interface to the SMI system, so no extra file dependancies exist.
...
class iViewXEyeTracker(EyeTracker):
    """iViewXEyeTracker class is the implementation of the pyEyeTracker
    common eye tracker interface for the SMI iViewX 1250 Hz eye trackers
    .. note:: 
        Only **one** instance of iViewXEyeTracker can be created within an experiment. Attempting to create > 1 instance will raise an exception. 
    ...
    def __init__(self,experimentContext):
        ...
        """
        if iViewXEyeTracker._INSTANCE is None:
            EyeTracker.__init__(self,experimentContext)
            self.eventHandler=iViewXEventHandler(self)  
        else:
            raise "iViewXEyeTracker instance already exists. del it or access it via getEyeTracker()"
    ...
    def sendCommand(self, command, value, wait=True):
        ...
        if wait is True:
            print "iViewXEyeTracker.sendCommand wait param not implemented"
    ...
    def getDataFilteringLevel(self,data_stream,value, **args):
        return ET_RTN_CODES.ET_NOT_IMPLEMENTED
        
    def setDataFilteringLevel(self,level,data_stream):
        ...
        if data_stream == ET_DATA_STREAMS.LINK or data_stream == ET_DATA_STREAMS.ANALOG:
            return ET_RTN_CODES.ET_NOT_IMPLEMENTED
...
class iViewXExtension(EyeTrackerVendorExtension):
    '''
